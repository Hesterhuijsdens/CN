...
plt.figure(2)
plt.plot(np.linspace(np.min(a) + 2, np.max(a) + 2, int(nr_of_bins)), bin_counters, label='sparse', color='green')
plt.plot(np.linspace(np.min(a_pca) + 2, np.max(a_pca) + 2, int(nr_of_bins_pca)), bin_counters_pca, label='pca', color='blue')
plt.xlabel('a_i')
plt.ylabel('p(a_j)')
plt.title('Frequencies of a using natural images')
plt.legend()
plt.show()