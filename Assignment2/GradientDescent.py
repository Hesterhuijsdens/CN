...
yloss = cost(ytest, t_test)
ycost = [float(yloss) for x in xaxis]

print "GRADIENT DESCENT"
print "Train class error: ", classification_error(ytrain, t)
print "Val class error: ", classification_error(yval, t_val)
print "Test class error: ", classification_error(ytest, t_test)
print " "
print "Train loss: ", tloss[n_epochs-1]
print "Val loss", vloss[n_epochs-1]
print "test loss: ", ycost
print " "
print "Train accuracy: ", testing(x, weight_vector, t), "%"
print "Val accuracy: ", testing(x_val, weight_vector, t_val), "%"
...
plt.figure()
plt.subplot(2, 2, 1)
plt.plot(xaxis, tloss, label='training loss')
plt.plot(xaxis, vloss, label='validation loss')
plt.plot(xaxis, ycost, label='test loss', linestyle='-.', c='lightgrey')
plt.legend(["train", "test"])
plt.title("Gradient descent (eta=%1.1f)" %eta)
plt.xlabel("N")
plt.ylabel("loss")
