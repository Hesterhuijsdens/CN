...
def plot_Cp(w, x_coords, y_coords):
    ...
    C = np.zeros((493, 493))
    allcoords = np.linspace(-1.0, 1.0, 493)
    all_combinations = []
    for pair in itertools.product(allcoords, repeat=2):
    ...
    C = C.flatten(order='C')
    print np.where(distance >= 1.0)
    print np.shape( np.where(distance >= 1.0))
    print np.shape(C[np.where(distance >= 1.0)])
...
def figure3(x_coords, y_coords, goal_x, goal_y, x_path, y_path, w, z): # to generate Figure 3 from Foster et al. (2000)
    fig = plt.figure(3)
    plt.subplot(2, 3, 1)
    plot_Cp(w[1,:], x_coords, y_coords)

    plt.subplot(2, 3, 2)
    plot_Cp(w[6, :], x_coords, y_coords)

    plt.subplot(2, 3, 3)
    plot_Cp(w[21, :], x_coords, y_coords)
